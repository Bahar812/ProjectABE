...
const EditOrder = () => {
  ...
  const { pesanan, total, totalSebelumReset } = useSelector(
  ...
  const [tokenPembayaran, setTokenPembayaran] = useState("hbcyhubqwcbqw");
  ...
  useEffect(() => {
    ...
    console.log("nilai setelahhh " + localtotal)// Convert total to string
      ...
      const handleButtonClick = async (button) => {
        console.log("uuidnya " + id + "button " + button);
        let total = 0;
        if(button === "simpanOrder"){
          ...
          if(total > 0){
            await axios.patch(`http://localhost:5000/transactions/${id}` ,{statusPembayaran, tipePesanan, total, tokenPembayaran, tipePembayaran, userType, receivedAmount, changeAmount });
      ...
      useEffect(() => {
        ...
        console.log("ini dia total dan pesanan " + total + pesanan )
        ...
        const getIdTransaction = async () => {
            await dispatch(getDetails({id : id}));
          try {
